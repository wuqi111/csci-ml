...
def forward(pi, A, B, O):
  ...
  for i in range(1,N):
    for j in range(S):
      alpha[j, i] = B[j,O[i]]*np.sum(A[t,j] *alpha[t,i -1] for t in range(S)) #what is the pro of jth state and 1th - tth X
...
def backward(pi, A, B, O):
  ...
  for t in reversed(range(N-1)):
    for m in range(S):
      beta[m,t] = np.sum(beta[j,t+1] * A[m,j] * B[j,O[t+1]] for j in range(S))
...
def viterbi(pi, A, B, O):
  ...
  for i in range(S):
  	if i == 0:
  		...
  		for j in range(N):
  			value1 = p[:,i-1]*A[:,j] * B[j,O[i]]
  			p[j,i] = np.max(value1)
  			s[j,i] = int(np.argmax(value1))
  temp = int(np.argmax(p[:,S-1]))
  path.append(temp)

  for i in range(S-2,-1,-1):
  	temp1 = int(s[path[-1],i])
  	path.append(temp1)
