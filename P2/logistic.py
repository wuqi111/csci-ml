...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(max_iterations):
   		...
   		# sigd = sigmoid(- (X.dot(w) * y + B))
   		# w = w + step_size * np.sum(X.T*sigd * y, axis = 1)
   		# b = b + step_size * sigd.T.dot(y)
   		h = sigmoid(X.dot(w) + b)
   		error = y - h
   		w = w + step_size*np.average(X.T*error,axis = 1)
   		b = b + step_size*np.average(error)
...
def multinomial_train(X, y, C, 
    ...
    for i in range(max_iterations):
    	# temp = np.zeros(shape = (C,D))
    	# B = np.zeros(shape = (C))

    	# for z in range(C):
    	data = w.dot(X.T) +  b.reshape(C,1) ##np.repeat(b.reshape(C,1),N,axis=1)
    	maxv = np.max(data, axis = 0)
    	# maxv = np.repeat(maxv.reshape(N,1),C,axis = 1).T
    	data = data - maxv
    	data = np.exp(data)
    	sum_exp = np.sum(data, axis = 0)
    	# sum_exp =np.repeat(sum_exp.reshape(N,1),C,axis = 1).T
    	data = data/sum_exp
    	for j in range(N):
    		for z in range(C):
    			if z == y[j]:
    	...
    	# 	p = np.zeros(shape = (C,))
    	# 	for z in range(C):
    	# 		maxx =np.max(w.dot(X[j,:]) + b)
    	# 		Max = np.array([maxx]*C)
    	# 		sumexp = np.sum(np.exp(w.dot(X[j,:]) + b - Max))
    	# 		print(np.exp(w.dot(X[j,:]) + b - Max))

    	# 		if z == y[j]:
    	# 			p[z] = np.exp(w[z,:].T.dot(X[j,:]) + b[z]  - maxx) / sumexp - 1
    	# 		else:
    	# 			p[z] =  np.exp(w[z,:].T.dot(X[j,:])  + b[z] - maxx) / sumexp

    	# 	temp += p.reshape(len(p),1).dot(X[j,:].reshape(1,len(X[j,:])))
    	# 	B += p
    	# w = w - step_size * temp
    	# b = b - step_size * B
   	# print(w,b)
...
def multinomial_predict(X, w, b):
    ...
    """   
    for i in range(N):
    	Everydata = w.dot(X[i,:])+b
    	predict = np.argmax(Everydata)
    	preds[i] = predict
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(C):
    	...
    	for z in range(max_iterations):
   			sigd = sigmoid(( X.dot(w[i,:]) + b[i]))
   			error = pre - sigd
   			w[i,:] = w[i,:] + step_size* np.average(X.T * error, axis = 1)
   			b[i] = b[i] + step_size * np.average(error) 


   		# 	h = sigmoid(X.dot(w) + np.array([b] * N))
   		# error = y - h
   		# w = w + step_size*np.average(X.T*error,axis = 1)
   		# b = b + step_size*np.average(error)
