...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # N,D = X.shape()
        # forward_output = np.array(shape = (N,output_D))
        # for i in range(N):
        # 	forward_output[i] = self.params['W'].T.dot(X[i]) + self.params['b']
    ...
    def backward(self, X, grad):
        ...
        # self.gradient['b'] = self.gradient['b'] - np.average(grad,axis = 0)

        # backward_output = grad.T.dot(X)
        self.gradient['W'] = X.T.dot(grad)
        self.gradient['b'] = np.sum(grad,axis = 0)#np.expand_dims(np.sum(grad, axis=0), axis = 0)
...
class relu:
    ...
    def forward(self, X):
        ...
        # for i in range(N):
        #     for j in range(D):
        #         forward_output[i,j] = np.max(0,X[i,j])
