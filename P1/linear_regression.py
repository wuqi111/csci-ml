...
def regularized_linear_regression(X, y, lambd):
  """
    ...
    """
  #####################################################
  Xi = np.linalg.inv(X.T.dot(X) + lambd*np.eye(X.shape[1]))
  w = Xi.dot(X.T).dot(y)
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  #####################################################
  for m in lambds:
    x_train = np.linalg.inv(Xtrain.T.dot(Xtrain)+m *np.eye(Xtrain.shape[1]))
    w_train = x_train.dot(Xtrain.T).dot(ytrain)

    err = np.sum(np.power(Xval.dot(w_train)-yval,2))
...
def test_error(w, X, y):
  """
    ...
    """
  [i,j] = X.shape
  err = np.sum(np.power(X.dot(w)-y,2))
  err = err / i
