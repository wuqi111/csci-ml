...
def objective_function(X, y, w, lamb):
    ...
    for i in range(len(obj_value)):
    	if obj_value[i] < 0:
    		obj_value[i] = 0
    obj_value = (lamb/2 * value) + np.mean(obj_value)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        y_try = []
        for i in A_t:
        	X_try.append(Xtrain[i])
        	y_try.append(ytrain[i])
        
        X_try = np.array(X_try)
        y_try = np.array(y_try)
        test_value = np.dot(X_try,w)
        pred = np.squeeze(test_value,axis = 1)*y_try
        ...
        w *= (1 - gt *lamb) 
        temp = np.expand_dims(y_try,axis = 1)
        summ = np.sum(temp*X_try , axis = 0)

        w +=  gt / k * np.expand_dims(summ,axis = 1)
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    yt = np.squeeze(np.dot(Xtest,w_l),axis = 1)
    ...
    for i in range(len(yt)):
    	if yt[i] < t:
    		...
    		yt[i] = 1
    test_acc = np.mean(np.array(ytest) == np.array(yt))
